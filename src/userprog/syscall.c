...
static bool is_valid_addr(const void *vaddr, bool write) {
  ...
  return (pagedir_get_page (cur->pagedir, vaddr) != NULL || page != NULL);
}

static bool get_user(const uint8_t *uaddr) {
...
static bool
valid_addr (struct thread *cur, void *vaddr, bool write)
{
  return ( is_user_vaddr (vaddr) &&
      is_valid_addr(vaddr, write) && get_user(vaddr));
}
 ...
 */
static bool
valid_buffer (struct thread *cur, char *vaddr, int lim, bool zeroed, bool write)
{
  int i = 0;
  bool found_zero = false;

  for (i = 0; i < lim; i++) {
    if ( !valid_addr (cur, vaddr+i, write)) {
      return false;
    }
    if ( zeroed && vaddr[i] == '\0' ) {
      found_zero = true;
...
read_syscall (struct syscall_signature *sig, struct thread *cur)
{
  ...
  struct file *f = NULL;
  int rv = -1;
  if ((uint32_t)buf > (uint32_t)org_esp) {
    void * newaddr = (uint32_t)org_esp & ~PGMASK;
    while ((uint32_t)newaddr < (uint32_t)buf + lim) {
      vm_install_page_stack(newaddr);
      newaddr += PGSIZE;
    }
  }
